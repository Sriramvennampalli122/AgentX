...
export function ExecutionPanel({ 
      ...
      if (segment.startsWith('```')) {
        ...
        return (
          <div key={idx} className="my-6">
            <div className="bg-black/60 rounded-t-xl px-4 py-2 border border-white/5 border-b-0 flex items-center justify-between">
              <span className="text-[9px] font-code text-white/40 uppercase tracking-widest">Source Code</span>
              <Button variant="ghost" size="sm" className="h-6 w-6 p-0 hover:bg-white/10" onClick={() => copyToClipboard(code.trim())}>
                <Copy className="h-3 w-3 text-white/40" />
              </Button>
            </div>
            <pre className="font-code text-xs bg-black/40 p-5 rounded-b-xl border border-white/5 overflow-x-auto text-emerald-400/90 leading-relaxed custom-scrollbar">
              {code.trim()}
            </pre>
          </div>
        );
  ...
  return (
    <div className="flex h-full w-full flex-col absolute inset-0">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col h-full overflow-hidden">
        <div className="px-8 bg-black/40 border-b border-white/5 shrink-0">
        ...
        <div className="flex-1 overflow-hidden relative">
          <TabsContent value="thinking" className="h-full m-0 overflow-y-auto custom-scrollbar data-[state=active]:flex flex-col" ref={scrollRef}>
            <div className="p-8 space-y-4 max-w-4xl mx-auto w-full pb-32">
                  ...
                  <div key={step.id} className="p-6 rounded-2xl bg-white/[0.01] border border-white/5 group hover:bg-white/[0.02] transition-colors">
                    <div className="flex gap-6">
                      ...
                      <div className="space-y-3">
                        ...
                        <p className="font-code text-sm leading-relaxed text-white/70 whitespace-pre-wrap">
                          {step.content}
                        </p>
                      </div>
                    </div>
          ...
          <TabsContent value="tools" className="h-full m-0 overflow-y-auto custom-scrollbar data-[state=active]:flex flex-col">
            <div className="p-8 grid gap-6 max-w-4xl mx-auto w-full pb-32">
              {toolCalls.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-[400px] gap-4 opacity-20">
                    ...
                    <div key={call.id} className="rounded-2xl border border-white/5 bg-white/[0.01] overflow-hidden">
                      ...
                      <div className="p-8 space-y-8">
                        <div className="space-y-3">
                          ...
                          <pre className="font-code text-xs bg-black/40 p-5 rounded-2xl border border-white/5 overflow-x-auto text-white/60 leading-relaxed custom-scrollbar whitespace-pre-wrap">
                            {call.content}
                          </pre>
                        </div>
                        {result && (
                          <div className="space-y-3">
                            ...
                            <div className="font-code text-xs p-5 rounded-2xl border border-white/5 bg-white/[0.01] text-white/40 leading-relaxed italic border-l-2 border-l-primary/30 whitespace-pre-wrap">
                              {result.content}
                            </div>
                          </div>
                        )}
          ...
          <TabsContent value="answer" className="h-full m-0 overflow-y-auto custom-scrollbar data-[state=active]:flex flex-col">
            <div className="p-12 max-w-4xl mx-auto space-y-12 pb-32 w-full">
              {!finalAnswer ? (
                ...
                <>
                  ...
                  <div className="prose prose-invert prose-p:leading-relaxed max-w-none text-white/70 leading-relaxed font-body text-[17px] space-y-10">
                    {renderContent(finalAnswer.content)}
                  </div>
